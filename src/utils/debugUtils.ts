...
const getLabelBackgroundColor = (debug: DebuggerUtils.Debugger): string => '\u001B[4' + (Number(debug.color) < 8 ? Number(debug.color) : '8;5;' + Number(debug.color)) + 'm';
...
export const setupDebugger = (config: SpeedGooseConfig): void => {
    ...
    const nameSpacesToEnable = modelsToDebug.flatMap(model => operationsToDebug.map(operation => `${DEFAULT_DEBUGGER_NAMESPACE}:${model}:${operation}`));
