...
export const getDocumentCacheKey = (modelName: string, id: string): string => `${CacheNamespaces.DOCUMENTS}:${modelName}:${id}`;
...
export const handleCachedPopulation = async <T extends Document>(
    ...
    for (const options of populateOptions) {
        ...
        const cacheKeys = idsToPopulate.map(id => getDocumentCacheKey(populatedModel.modelName, id));
        const docsFromCache = await cacheStrategy.getDocuments(cacheKeys);

        // Identify missing IDs
        const missedIds = idsToPopulate.filter(id => !docsFromCache.has(getDocumentCacheKey(populatedModel.modelName, id)));
        ...
        for (const doc of documents) {
            ...
            let value = Array.isArray(ids)
                ? ids.map(id => docsFromCache.get(getDocumentCacheKey(populatedModel.modelName, id.toString())))
